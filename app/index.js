...
const app = express();
const nodeidpath = path.join(config.getHomeDir(),".nodeid");
...
app.get('/api/peers',(req, res, next)=>{
  return res.json({"peers":peers.getPeers(),"status":peers.getStatus()});
});
...
http.listen(HTTP_PORT, () => console.log(`Listening on port ${HTTP_PORT}`));
peers.listen();
//p2pServer.listen();
//p2pClient.listen();
